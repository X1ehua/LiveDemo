	...
	JNIEnv *env, jobject thiz, jstring url_, jboolean isPublishMode)
{
	...
	if (!ret)
	{
		RTMP_Free(rtmp);
		rtmp = NULL;
		LOGE("RTMP_Connect() failed, ret %d", ret);
		return NULL;
	}
	...
	if (!ret)
	{
		LOGE("RTMP_ConnectStream() failed, ret %d", ret);
		// ret = RTMP_ConnectStream(rtmp, 0);
		RTMP_Close(rtmp);
		RTMP_Free(rtmp);
		rtmp = NULL;
	...
	JNIEnv *env, jobject thiz, jlong rtmp, jbyteArray data, jint size, jint type, jint ts)
{
	...
	if (!ret)
	{
		LOGW("end write error %d", sockerr);
		return sockerr;
	}
	else
	{
 ...
 * Signature: ()I
 */
JNIEXPORT jint JNICALL Java_me_lake_librestreaming_rtmp_RtmpClient_close(JNIEnv *env, jlong rtmp, jobject thiz)
{
...
JNIEXPORT jstring JNICALL Java_me_lake_librestreaming_rtmp_RtmpClient_getIpAddr(JNIEnv *env, jobject thiz, jlong rtmp)
{
	if (rtmp != 0)
	{
		RTMP *r = (RTMP *)rtmp;
